...
//! Vesta elliptic-curve host functions.
...
/// Computes a variable-base MSM through the native host interface.
///
/// The caller remains responsible for deciding how the resulting point affects
/// proof validity.
pub fn msm(bases: &[Vesta], scalars: &[Fp]) -> Result<ProjectiveVesta, ()> {
    ...
    let result = host_calls::vesta_msm(&bases, &scalars)?;
...
/// Native interfaces for Vesta elliptic-curve operations.
#[runtime_interface]
pub trait HostCalls {
    /// Variable-base multi-scalar multiplication on Vesta.
    ///
    /// - `bases`: `ArkScale<Vec<Vesta>>`
    /// - `scalars`: `ArkScale<Vec<Fp>>`
    /// - returns: `ArkScaleProjective<ProjectiveVesta>`
    #[allow(clippy::result_unit_err)]
    fn vesta_msm(bases: &[u8], scalars: &[u8]) -> Result<Vec<u8>, ()> {
        utils::native::msm_sw::<VestaParameters>(bases, scalars)
    }
...
mod tests {
    ...
    #[test]
    fn native_msm_matches_arkworks() {
        let scalars: Vec<Fp> = [1_u64, 2, 3, 5, 8].into_iter().map(Into::into).collect();
        let bases = vec![Vesta::generator(); scalars.len()];

        let expected = ProjectiveVesta::msm(&bases, &scalars).expect("matching input lengths");
        let actual = msm(&bases, &scalars).expect("native MSM succeeds");

        assert_eq!(actual, expected);
    }

    #[test]
    fn native_msm_rejects_mismatched_lengths() {
        assert!(msm(&[Vesta::generator()], &[]).is_err());
    }
