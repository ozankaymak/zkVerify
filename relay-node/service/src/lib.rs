...
pub fn prewarm_native_verifier_parameters() -> Result<(), Error> {
    const KIMCHI_VESTA_SRS_LOG2: u8 = 16;
    const KIMCHI_DOMAIN_LOG2_SIZES: &[u8] = &[10, 11, 12, 13, 14, 15, 16];

    let started_at = Instant::now();
    log::info!(
        "Prewarming Kimchi native verifier parameters for Vesta SRS 2^{KIMCHI_VESTA_SRS_LOG2}"
    );
    native::prewarm_kimchi_builtin_srs(KIMCHI_VESTA_SRS_LOG2, KIMCHI_DOMAIN_LOG2_SIZES)
