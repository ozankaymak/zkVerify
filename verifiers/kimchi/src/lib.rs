...
type NativeOpeningProof = accelerated_opening::AcceleratedOpeningProof;
...
fn prepare_verifier_index(
    ...
    let srs = BuiltinSrs::new(srs_id, verifier_index.max_poly_size, verifier_index.public);
    srs.validate_native_access(domain_size)
        .map_err(|_| VerifyError::InvalidVerificationKey)?;
