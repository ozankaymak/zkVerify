...
//! IPA opening verification backed by the native fixed Pallas15 SRS MSM.
...
#[serde(transparent)]
pub struct BuiltinOpeningProof(OpeningProof<Pallas, FULL_ROUNDS>);

impl BuiltinOpeningProof {
    pub(crate) fn from_standard(proof: OpeningProof<Pallas, FULL_ROUNDS>) -> Self {
        Self(proof)
    }
...
impl OpenProof<Pallas, FULL_ROUNDS> for BuiltinOpeningProof {
    type SRS = BuiltinSrs;

    #[cfg(feature = "std")]
    fn open<EFqSponge, RNG, D: EvaluationDomain<Fq>>(
        _srs: &Self::SRS,
        _group_map: &<Pallas as poly_commitment::commitment::CommitmentCurve>::Map,
        _plnms: &[(DensePolynomialOrEvaluations<'_, Fq, D>, PolyComm<Fq>)],
        _elm: &[Fq],
        _polyscale: Fq,
        _evalscale: Fq,
        _sponge: EFqSponge,
    ...
    where
        EFqSponge: Clone + FqSponge<Fp, Pallas, Fq, FULL_ROUNDS>,
    ...
    fn verify<EFqSponge, RNG>(
        srs: &Self::SRS,
        group_map: &<Pallas as poly_commitment::commitment::CommitmentCurve>::Map,
        batch: &mut [BatchEvaluationProof<Pallas, EFqSponge, Self, FULL_ROUNDS>],
        rng: &mut RNG,
    ) -> bool
    where
        EFqSponge: FqSponge<Fp, Pallas, Fq, FULL_ROUNDS>,
        RNG: RngCore + CryptoRng,
    {
...
// Pallas, validates the fixed-SRS shape, and partitions the final MSM so the built-in SRS terms can
...
fn verify_with_native_srs_msm<EFqSponge, RNG>(
    srs: &BuiltinSrs,
    group_map: &<Pallas as poly_commitment::commitment::CommitmentCurve>::Map,
    batch: &mut [BatchEvaluationProof<Pallas, EFqSponge, BuiltinOpeningProof, FULL_ROUNDS>],
    rng: &mut RNG,
) -> bool
where
    EFqSponge: FqSponge<Fp, Pallas, Fq, FULL_ROUNDS>,
    RNG: RngCore + CryptoRng,
{
    ...
    let mut h_scalar = Fq::zero();
    let mut g_scalars = vec![Fq::zero(); srs_len];
    let mut extra_bases = Vec::new();
    let mut extra_scalars = Vec::new();

    let rand_base = Fq::rand(rng);
    let sg_rand_base = Fq::rand(rng);
    let mut rand_base_i = Fq::one();
    let mut sg_rand_base_i = Fq::one();
    ...
    } in batch.iter_mut()
    {
        ...
        sponge.absorb_fr(&[shift_scalar::<Pallas>(*combined_inner_product)]);
    ...
    let result = match srs.profile() {
        Some(crate::PicklesProfileId::PicklesV1) => {
            native::pallas::pallas15_srs_msm(h_scalar, &g_scalars, &extra_bases, &extra_scalars)
        }
        None => return false,
    };
